...
class TextEncoder(nn.Module):
    def __init__(self, config):
        ...
        print('DEBUG: bert_model: %s' % (config.ptm_model))
    ...
    def get_bert_vec(self, text, text_mask, text_pos=None):
        if text_pos is None:
            ...
            text_vecs = self.bert(text, text_mask)
            # print('DEBUG: bert_result type = %s' % type(text_vecs))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[0]), text_vecs[0]))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[1]), text_vecs[1]))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[2]), text_vecs[2]))
        else:
            # _, _, text_vecs = self.bert(text, text_mask, position_ids=text_pos)
            text_vecs = self.bert(text, text_mask, position_ids=text_pos)
            # print('DEBUG: bert_result type = %s' % type(text_vecs))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[0]), text_vecs[0]))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[1]), text_vecs[1]))
            # print('DEBUG: type = %s, content = %s' % (type(text_vecs[2]), text_vecs[2]))
        text_vecs = list(text_vecs[2])
