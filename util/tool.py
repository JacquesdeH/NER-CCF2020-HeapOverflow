...
def json_dump_by_line(data, file):
    # TODO: skip convert 2 json
    
    # for path in ['test_data.json', 'train_data.json', 'clue']:
    #     if path in file:
    #         return
    
    # print('DEBUG: dump FILE = ' + file)
    # with open(file, 'r') as f:
    #     print('DEBUG: len1 = %d' % len(f.readlines()))
    fp = open(file, 'w+', encoding='utf-8')
    
    # print('DEBUG: data_len: %d' % len(data))
    ...
    fp.close()
    # with open(file, 'r') as f:
    #     print('DEBUG: len2 = %d' % len(f.readlines()))
...
def json_load_by_line(file):
    # print('DEBUG: load_by_line FILE = ' + file)
    data = []
    fp = open(file, 'r', encoding='utf-8')
    count = 0
    ...
    with Manager() as manager:
        ...
        for d in out_list:
            for key in out_data.keys():
                print('DEBUG: key: %s, len: %d' % (key, len(d[key])))
